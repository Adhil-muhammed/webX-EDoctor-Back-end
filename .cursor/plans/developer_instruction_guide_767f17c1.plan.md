---
name: Developer Instruction Guide
overview: A complete, prescriptive developer instruction guide for the ente-doctor-backend pnpm monorepo, covering architecture, commands, conventions, and code generation rules for all six NestJS microservices.
todos: []
isProject: false
---
...
- **Language:** TypeScript 5.7+, targeting ES2023 with `NodeNest` module resolution.
...
`**@ente-doctor/common`** — `packages/common/src/index.ts`
...
`**@ente-doctor/contracts**` — `packages/contracts/src/index.ts`
...
`**@ente-doctor/event-bus**` — `packages/event-bus/src/index.ts`
